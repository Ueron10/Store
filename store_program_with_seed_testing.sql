...
CREATE TABLE IF NOT EXISTS StockBatch (
  ...
  INDEX IX_StockBatch_PurchaseDate (PurchaseDate),
...
INSERT INTO StockBatch (Id, ProductId, Quantity, PurchaseDate, ExpiryDate, UnitCost, DiscountPercent)
VALUES
  -- Indomie: ada 1 batch mendekati kadaluarsa (diskon), 1 batch jauh (tanpa diskon)
  (@B_MIE_NEAR,  @P_MIE,   40, TIMESTAMP(DATE_SUB(@TODAY, INTERVAL 30 DAY), '08:00:00'), DATE_ADD(@TODAY, INTERVAL 10 DAY),  2500.00, 15.00),
  (@B_MIE_FAR,   @P_MIE,   80, TIMESTAMP(DATE_SUB(@TODAY, INTERVAL 25 DAY), '08:30:00'), DATE_ADD(@TODAY, INTERVAL 120 DAY), 2500.00, NULL),

  -- Teh: batch jauh
  (@B_TEH_FAR,   @P_TEH,   90, TIMESTAMP(DATE_SUB(@TODAY, INTERVAL 22 DAY), '09:00:00'), DATE_ADD(@TODAY, INTERVAL 180 DAY), 3500.00, NULL),

  -- Susu: 1 batch warning (diskon), 1 batch jauh
  (@B_SUSU_WARN, @P_SUSU,  12, TIMESTAMP(DATE_SUB(@TODAY, INTERVAL 18 DAY), '10:00:00'), DATE_ADD(@TODAY, INTERVAL 12 DAY),  6000.00, 20.00),
  (@B_SUSU_FAR,  @P_SUSU,  50, TIMESTAMP(DATE_SUB(@TODAY, INTERVAL 16 DAY), '10:30:00'), DATE_ADD(@TODAY, INTERVAL 200 DAY), 6000.00, NULL),

  -- Yogurt: 1 batch warning (diskon), 1 batch jauh
  (@B_YOG_WARN,  @P_YOG,   18, TIMESTAMP(DATE_SUB(@TODAY, INTERVAL 14 DAY), '11:00:00'), DATE_ADD(@TODAY, INTERVAL 7 DAY),   8000.00, 30.00),
  (@B_YOG_FAR,   @P_YOG,   20, TIMESTAMP(DATE_SUB(@TODAY, INTERVAL 12 DAY), '11:30:00'), DATE_ADD(@TODAY, INTERVAL 90 DAY),  8000.00, NULL),

  -- Roti: 1 batch expired (diskon), 1 batch near expiry (diskon)
  (@B_ROTI_EXP,  @P_ROTI,   6, TIMESTAMP(DATE_SUB(@TODAY, INTERVAL 6 DAY),  '13:00:00'), DATE_SUB(@TODAY, INTERVAL 2 DAY), 11000.00, 50.00),
  (@B_ROTI_NEAR, @P_ROTI,  15, TIMESTAMP(DATE_SUB(@TODAY, INTERVAL 5 DAY),  '13:30:00'), DATE_ADD(@TODAY, INTERVAL 3 DAY), 11000.00, 25.00),

  -- Sosis/Air/Kopi: jauh
  (@B_SOSIS_FAR, @P_SOSIS, 25, TIMESTAMP(DATE_SUB(@TODAY, INTERVAL 9 DAY),  '14:00:00'), DATE_ADD(@TODAY, INTERVAL 60 DAY),  20000.00, NULL),
  (@B_AIR_FAR,   @P_AIR,  120, TIMESTAMP(DATE_SUB(@TODAY, INTERVAL 8 DAY),  '15:00:00'), DATE_ADD(@TODAY, INTERVAL 365 DAY),  2000.00, NULL),
  (@B_KOPI_FAR,  @P_KOPI, 150, TIMESTAMP(DATE_SUB(@TODAY, INTERVAL 7 DAY),  '16:00:00'), DATE_ADD(@TODAY, INTERVAL 300 DAY),  1500.00, NULL);
